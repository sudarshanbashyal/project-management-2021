<?php
	...
	$query = "SELECT product.product_id, product.product_name, product.product_price, product.product_image, product.stock, product.disabled, users.user_name, rating.rating_star FROM product, users, shop, trader_category, rating WHERE product.product_id = rating.product_id && product.shop_id = shop.shop_id  && shop.user_id = users.user_id && users.user_id = rating.user_id && users.category_id = trader_category.category_id";
